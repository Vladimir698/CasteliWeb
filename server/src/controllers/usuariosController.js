...
exports.crear = async (req,res) => {
  try {
    ...
    if(!nombre||!usuario||password.length<4) return res.status(400).send('Nombre, usuario y una contraseña de al menos 4 caracteres son obligatorios.');
    ...
    await db.Usuario.create({nombre,usuario,email:String(req.body.email||'').trim()||null,passwordHash:await bcrypt.hash(password,10),rolId,activo:true});
